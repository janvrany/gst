			    ...
			    OOP method_class)
{
  ...
  _gst_sample_counter++;
  hashIndex = METHOD_CACHE_HASH (sendSelector, method_class, _gst_this_method);
  methodData = &method_cache[hashIndex];

  if UNCOMMON (METHOD_CACHE_DATA_MATCH(methodData, sendSelector, method_class, _gst_this_method ))
    {
      /* :-( cache miss )-: */
